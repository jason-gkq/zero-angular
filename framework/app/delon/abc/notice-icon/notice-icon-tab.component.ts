...
@Component({
  ...
  <div class="clear" (click)="onClear()">{{ data.clearText || '查看之前的消息' }}</div>
  <ng-template #listTpl>
      ...
      <ng-template #item let-item>
        <nz-list-item (click)="onClick(item)" [ngClass]="{'read': item.read}">
          <nz-list-item-meta
            ...
            <ng-template #nzTitle>
                <span style="display: inline-block;width: 400px;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;line-height: 1;">{{item.msgContent}}</span>
