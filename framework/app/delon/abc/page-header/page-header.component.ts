...
@Component({
    ...
    template: `
        <ng-container *ngIf="!breadcrumb; else breadcrumb">
            <nz-breadcrumb *ngIf="paths && paths.length > 0" [nzSeparator]="separator">
                <nz-breadcrumb-item *ngFor="let i of paths">
                    <ng-container *ngIf="i.link"><a [routerLink]="i.link" [queryParams]="i.queryParams">{{i.title}}</a></ng-container>
...
export class PageHeaderComponent implements OnInit, OnChanges, AfterViewInit {
    ...
    genBreadcrumb() {
        ...
        if (this._pathData.length > 0) {
            this.paths = this._pathData;
            return;
        }
        ...
        const menus = this.menuSrv.getPathByUrl(this.route.url || data.auth);
